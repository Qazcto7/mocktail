...
int main(int argc, char* argv[]) {
  // Roblox's own runtime constructs std::locale("") (the process's
  // environment locale) during its startup sequence. If LANG/LC_* names
  // a locale that isn't actually generated on the host (common on
  // minimal/container-style Linux installs), that construction throws
  // "collate_byname<char>::collate_byname failed to construct" and
  // Roblox does not catch it, crashing with "Roblox closed unexpectedly
  // because of an internal error" before anything is even on screen
  // (reported as issue #81). Probe the environment's locale here with
  // the POSIX C API - newlocale() returns nullptr on failure instead of
  // throwing, which matters because this project is built with
  // exceptions disabled - before anything else runs, and fall back to
  // a locale guaranteed to exist on any glibc system if the
  // environment's own locale can't be built. This way both Mocktail's
  // own code and libroblox.so inherit a safe LANG/LC_ALL.
  locale_t probe_env = ::newlocale(LC_ALL_MASK, "", static_cast<locale_t>(0));
  if (probe_env == static_cast<locale_t>(0)) {
    const char* fallback = "C.UTF-8";
    locale_t probe_fallback =
    ::newlocale(LC_ALL_MASK, fallback, static_cast<locale_t>(0));
    if (probe_fallback == static_cast<locale_t>(0)) {
      fallback = "C";
    } else {
      ::freelocale(probe_fallback);
    }
    ::setenv("LANG", fallback, 1);
    ::setenv("LC_ALL", fallback, 1);
    std::cerr << "[runtime] Environment locale is not installed on this "
    "system; falling back to "
    << fallback << " to avoid a startup crash\n";
  } else {
    ::freelocale(probe_env);
  }
                                                              ...
                                                              failure_dialog.SetMessage(
                                                                ...
                                                                return EXIT_FAILURE;
                                                                  ...
                                                                  if (command_line.options.mode == mocktail::runtime::CommandMode::kRun) {
                                                                    ...
                                                                    // mocktail_runtime (where RobloxWebViewBridge lives) cannot link
                                                                    // mocktail_window directly - mocktail_window already depends on
                                                                    // mocktail_runtime, so the reverse would be circular. main.cc links
                                                                    // both, so it wires the two together here instead.
                                                                    mocktail::runtime::SetRaiseWindowCallback(&mocktail::window::RaiseAndFocus);
                                                                  }
