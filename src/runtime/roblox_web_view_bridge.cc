...
void (*g_raise_window_callback)() = nullptr;
  ...
  for (; processed < events.size() && status.ok();) {
    ...
    switch (event.type) {
      ...
      case HostWindowEventType::kDataModelFocused:
        ...
        if (status.ok()) {
          std::fprintf(stderr,
                       ...
                       "Roblox\n");
          // The engine now believes it has focus, but the WebView that
          // just closed ran in a separate OS process/window; the window
          // manager does not automatically hand keyboard/mouse focus
          // back to the main window just because that broadcast fired.
          // Raise it explicitly so input actually reaches Roblox.
          if (g_raise_window_callback != nullptr) g_raise_window_callback();
        }
        break;
      case HostWindowEventType::kMessageBusWindowClose: {
        ...
        // The message above only tells Lua-level UI code the window is
        // gone; it does not tell the native input/rendering layer that
        // focus is back on the game view. Without this, actions taken
        // in the closed WebView (e.g. pressing Join in the Servers
        // list) stayed stuck until something else - such as the user
        // manually clicking the main window - incidentally restored
        // focus. Broadcast it explicitly here, same as the default
        // kDataModelFocused path.
        if (status.ok()) {
          status = BroadcastDataModelFocus(env, "Focused");
          if (status.ok()) {
            std::fprintf(stderr,
                         "  [webview] DataModel web surface focus returned "
                         "to Roblox\n");
            if (g_raise_window_callback != nullptr) g_raise_window_callback();
          }
        }
...
void RobloxWebViewBridge::HostWindowExited(void *context) {
  ...
  switch (exit_context->target->active_source) {
    case OpenSource::kNativeOverlay:
      // This used to reset active_generation/browser_login_fallback
      // here *and* set queue_close_event = false, which (a) skipped the
      // kDataModelFocused broadcast entirely, leaving the game's
      // DataModel thinking it never regained focus after a native
      // overlay (e.g. the Servers/Join list) closed, and (b) made the
      // deferred cleanup in DrainHostWindowEvents a no-op for this
      // source, since it only fires when active_generation still
      // matches event.generation - which an early reset to 0 prevents.
      // Native overlays now behave like every other WebView source:
      // the state reset and the focus broadcast both happen together,
      // once the queued event is actually drained.
...
void SetRaiseWindowCallback(void (*callback)()) {
  g_raise_window_callback = callback;
}
